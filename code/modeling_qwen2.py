...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        mean_attnn = attn_weights.mean(dim=-2)
        attn_weights_clone = attn_weights.clone()
        attn_weights_clone[:,:,16:18,:] = attn_weights_clone[:,:,16:18,:] + 0.01 * mean_attn.unsqueeze(-2)
        attn_weights_clone[:,:,:,16:18] = attn_weights_clone[:,:,:,16:18] + 0.01 * mean_attnn.unsqueeze(-1)
        attn_weights = attn_weights_clone
        """
...
def reject_sampling(probs_tensor, num_samples, device): 
    n = probs_tensor.shape[0]
    M = torch.max(probs_tensor * n)
    
    samples = []
    for _ in range(num_samples):
        while True:
            index = torch.randint(low=0, high=n, size=(1,), device=device)
            p = probs_tensor[index]
            
            accept_prob = (p * n) / M
            u = torch.rand(1, device=device)
            if u < accept_prob:
                samples.append(index.item())
                break
    return torch.tensor(samples).to(device)


def reject_sampling_tensor(probs, k):
    samples = []
    for i in range(probs.shape[0]):
        samples_temp = []
        for j in range(probs.shape[1]):
            prob_temp = probs[i,j,:]
            samples_temp.append(reject_sampling(prob_temp, k, prob_temp.device))
        samples.append(torch.stack(samples_temp, dim=0))
